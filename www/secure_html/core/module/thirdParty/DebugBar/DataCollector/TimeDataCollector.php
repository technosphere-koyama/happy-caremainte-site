...
class TimeDataCollector extends DataCollector implements Renderable
{
     ...
     * @param string|null $microtime set time by user (add 20150820)
     * @param string|null $collector The source of the collector
     */
    public function startMeasure($name, $label = null, $microtime = null, $collector = null)
    {
	if (!empty($microtime)) {
		$start = $microtime;
	} else {
     ...
     */
    public function stopMeasure($name, $microtime = null, $params = array())
    {
	if (!empty($microtime)) {
		$end = $microtime;
	} else {
