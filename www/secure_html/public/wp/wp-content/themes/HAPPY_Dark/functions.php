...
================================================================================
  スクリプト追加
  通常のCSSは分岐の中で<body>のidを設定しているのでheader.phpに残す
================================================================================
 */
function add_my_scripts() {
  ...
  wp_register_script( 'jquery', $template_directory_uri.'/lib/jquery-3.3.1.min.js', array(), null, false );
  wp_register_script( 'slick', $template_directory_uri.'/lib/slick/slick.min.js', array( 'jquery' ), null, $in_footer);
  wp_register_script( 'scrollreveal', 'https://unpkg.com/scrollreveal', array(), null, $in_footer);
  wp_register_script( 'chart-js', 'https://cdn.jsdelivr.net/npm/chart.js@2.9.3/dist/Chart.min.js', array(), null, $in_footer );

  wp_enqueue_style( 'slick', $template_directory_uri.'/lib/slick/slick.css' );
  wp_enqueue_style( 'slick-theme', $template_directory_uri.'/lib/slick/slick-theme.css' );

  wp_enqueue_script( 'happy-common', $template_directory_uri.'/js/common.js', array('jquery'), null, $in_footer );
  wp_enqueue_script( 'slick_common', $template_directory_uri . '/js/slick_common.js', array('slick'), null, $in_footer );
  $theme_url = array(
    'template_directory_uri' => $template_directory_uri,
  );
  wp_localize_script( 'slick_common', 'theme_url', $theme_url );

  if ( is_page('mypage') ) {
    wp_enqueue_script( 'happy-mypage', $template_directory_uri.'/js/mypage.js', array('chart-js'), null, $in_footer );
  }

  if ( is_front_page() ) {
    wp_enqueue_style( 'happy-vegas', $template_directory_uri.'/lib/vegas/vegas.min.css' );
    wp_enqueue_script( 'happy-vegas', $template_directory_uri.'/lib/vegas/vegas.min.js', array('jquery', 'scrollreveal'), null, $in_footer );
    wp_enqueue_script( 'happy-front-page', $template_directory_uri.'/js/front-page.js', array('jquery', 'scrollreveal'), null, $in_footer );
/*追加　2021.03.02*/
    wp_enqueue_script( 'custom_js', $template_directory_uri.'/js/custom.js?210326', array('jquery', 'scrollreveal'), null, $in_footer );

  } else {
    wp_enqueue_script( 'happy-non-front-page', $template_directory_uri.'/js/non-front-page.js', array('jquery'), null, $in_footer );
  }

  $site_root_uri = str_replace( '/wp', '', get_home_url() );
  if(is_post_type_archive('shop') || is_tax('list') || is_singular( 'shop' ) || is_page('shop_search') ) {
    wp_enqueue_script( 'happy-assets-shop', $site_root_uri.'/_assets/js/shop.js', array('jquery'), null, $in_footer );
  } elseif(is_post_type_archive('technology') || is_tax('technology_cat') || is_singular( 'technology' ) ) {
    wp_enqueue_script( 'happy-assets-technology', $site_root_uri.'/_assets/js/technology.js', array('jquery'), null, $in_footer );
  }
}
...
/*
================================================================================
	YouTube ショートコード のHTMLを整形
================================================================================
*/
function my_embed_oembed_html( $html, $url, $attr, $post_ID ) {
  if ( strpos( $html, 'youtube.com' ) !== FALSE ) {
    return "<div class='movie movie-responsive'>".$html."</div>";
  }
  return $html;
}
...
================================================================================
	「さがす」へのリンク　ショートコード
	件数を書き換えるときに便利なようにショートコード化
================================================================================
*/
function my_shortcode_carementeh_search() {
  return '<a href="/carementeh_search/" class="l2">2000 を超える before&amp;after の実例を見る</a>';
}
add_shortcode('carementeh_search_button', 'my_shortcode_carementeh_search');
...
function is_custom_mobileCheck() {
	...
	if(preg_match('/iPhone/i', $ua) || preg_match('/Android/i', $ua) || preg_match('/iPod/i', $ua) || preg_match('/CareMenteh App User/i', $ua)) {
		return true;
	}else{
		return false;
	}
	...
	if(!is_home()&&!is_admin()){
		...
		if(is_single()){
			if( is_singular( 'carementeh' ) ){
				...
				$str.='<p class="pankuzu"><a href="'.home_url('/').'carementeh/">サービス・技術</a></p>';
				...
				$cat = $categories[0];
        if($cat->slug == 'before_after'){
          $str.='<p class="pankuzu"><a href="'.home_url('/').'technology/before_after/list/">事例・レビュー</a></p>';
          $str.='<p class="pankuzu"><a href="'.home_url('/').'technology/before_after/list/">Before&After /ケアメンテ事例集</a></p>';
        } else {
          $str.='<p class="pankuzu"><a href="'.home_url('/').'carementeh/">サービス・技術</a></p>';
          if($cat->slug == 'weightlessness'){
            $str.='<p class="pankuzu"><a href="'.home_url('/').'technology/data/weightlessness/hohenstein/">技術の検証データ</a></p>';
          } else {
            $str.='<p class="pankuzu"><a href="'.home_url('/').'technology/">ケアメンテ&reg;を支える技術</a></p>';
          }
        }
        ...
        $str.='<p class="pankuzu">特集・読みもの</p>';
				$str.='<p class="pankuzu"><a href="'.home_url('/').'knowledge/">おしゃれの雑学</a></p>';

			}elseif( is_singular( 'detail' ) ){ /*こだわりのディテール　シングルページ*/
        $str.='<p class="pankuzu">特集・読みもの</p>';
				$str.='<p class="pankuzu"><a href="'.home_url('/').'detail/">こだわりのディテール</a></p>';

			}elseif( is_singular( 'corporate' ) ){ /*会社情報　シングルページ*/
				$categories = get_the_terms($post->ID,'corporate_cat');
				$cat = $categories[0];
				$str.='<p class="pankuzu"><a href="'.home_url('/').'corporate/">会社情報・リクルート</a></p>';
      ...
      if( get_the_ID() == 28 || get_the_ID() == 1039 ){
        $str.='<p class="pankuzu">特集・読みもの</p>';
        if( get_the_ID() == 28 ) {
          $str.='<p class="pankuzu"><a href="'.home_url('/').'knowledge/">おしゃれの雑学</a></p>';
        } else {
