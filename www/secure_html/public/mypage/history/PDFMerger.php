...
class PDFMerger
{
	...
	public function addPDF($filepath, $pages = 'all')
	{
		...
		{
			$_SESSION['error_download'] = "Could not locate PDF";
	 ...
	 * @return PDF
	 */
	public function merge($outputmode = 'browser', $outputpath = '納品書添付資料.pdf')
	{
		if(!isset($this->_files) || !is_array($this->_files)) {
			// throw new exception("No PDFs to merge."); 
			// echo "<script>window.close();</script>";
			$_SESSION['error_download'] = "No PDFs to merge.";
			echo "<script>window.close();</script>";
		} 
    ...
    $fpdi->SetPrintFooter(false);
			...
			else
			{
				foreach($filepages as $page)
				{
					if(!$template = $fpdi->importPage($page)) {
						...
						$_SESSION['error_download'] = "Could not load page"; // '$page' in PDF '$filename'. Check that the page exists";
						echo "<script>window.close();</script>";
					}
					$size = $fpdi->getTemplateSize($template);
					$orientation = ($size['h'] > $size['w']) ? 'P' : 'L';
		...
		else
		{
			...
			else
			{
				...
				// return false;
				$_SESSION['error_download'] = "Error outputting PDF.";
				echo "<script>window.close();</script>";
